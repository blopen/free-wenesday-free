...
class Binance:
    ...
    def my_balance(self):
        print(self.streamObject.get_account(recvWindow=60000))
...
if avg_price > ask_price:
  ...
  try:
    ...
    if check_symbol=="LTCBUSD":
            ...
            order = binance.my_trades_btc()
            market_order = binance.streamObject.order_limit_buy(symbol=thecurrency[bestCur], quantity=0.0005, price=ask_price)#working
    elif check_symbol=='ETHUSDT':
            order = binance.my_trades_eth()
            market_order = binance.streamObject.order_limit_buy(symbol=thecurrency[bestCur], quantity=0.007, price=ask_price)  
...
if avg_price > ask_price:
  ...
  try:
    ...
    if check_symbol=="LTCBUSD":
        order = binance.my_trades_lct()
        market_order = binance.streamObject.order_market_sell(symbol=thecurrency[worestCur], quantity=0.4)#crush by insuflienc
    if check_symbol=='ETHUSDT':
        order = binance.my_trades_eth()
        market_order = binance.streamObject.order_market_sell(symbol=thecurrency[worestCur], quantity=0.05)#crush by insuflienc
    if check_symbol=='BTCBUSD':
        order = binance.my_trades_btc()
        market_order = binance.streamObject.order_market_sell(symbol=thecurrency[worestCur], quantity=0.0005)
...
prRed(binance._get_time_offset())
